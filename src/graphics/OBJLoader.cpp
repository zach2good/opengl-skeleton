...
Mesh OBJLoader::LoadFromFile(const char* fileName, const char* texFileName)
{
	...
	std::vector<GLuint> indices;
		...
		// Create Vector of Vertex-Structs from information
		std::vector<Vertex> vertexStructs = std::vector<Vertex>();
		for (int i = 0; i < vertices.size(); i++)
		{
			Vertex temp;
			temp.Position = vertices.at(i);
			temp.Normal = normals.at(i);
			temp.TexCoords = textures.at(i);
			vertexStructs.push_back(temp);
		}

		std::vector<Texture> texs = std::vector<Texture>();
		Texture t = Texture(texFileName);
		texs.push_back(t);

		Mesh m = Mesh(vertexStructs, indices, texs);
		printf("Load time: %dms\n", clock() - startTime);

		return m;
}

void OBJLoader::ProcessVertices(char* vertexData, std::vector<GLuint>& indices, std::vector<glm::vec2>& tempTextures,
